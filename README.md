...
#### 性能基準は p95 25 ms 未満です

当初の目標であった **p95 10 ms 未満は達成していません**。このマシンではプロセスの起動そのものが約 11.8 ms かかり（`cat` と `mv` だけの `/bin/sh` スクリプトでも 11.4 ms）、fork+exec 型の hook である限り 10 ms は物理的に到達不能です。

そのため初期実用基準を **p95 25 ms 未満** としています。実測は p50 16.5 ms / p95 17.2 ms（コンテナ内 200 回）で基準内です。

10 ms 未達を許容する根拠は、変更前 268 ms からの大幅改善に加えて、**クリティカルパス上の SQLite オープンと fsync がどちらもゼロになった**ことです。残っているコストはプロセス起動が支配的で、ディスク性能には依存しません。
